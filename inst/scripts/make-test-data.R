...
# Script to extract a miniature subset of curatedBreastData objects and save them as an archive for Zenodo
...
devtools::load_all()
esets <- getCuratedBreastDataExprSetList()
clin <- getClinicalData()

# Create a small subset
message("Subsetting data...")
curatedBreastDataExprSetList <- esets[1:2]
clinicalData <- clin
clinicalData$clinicalTable <- clinicalData$clinicalTable[rownames(clinicalData$clinicalTable) %in% c(sampleNames(curatedBreastDataExprSetList[[1]]), sampleNames(curatedBreastDataExprSetList[[2]])), ]
...
message(sprintf("Done! The test archive has been saved to %s", archive_name))
